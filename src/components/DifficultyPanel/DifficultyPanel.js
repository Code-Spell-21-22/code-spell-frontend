...
const DifficultyPanel = (props) => {
    ...
    const [clickBehavior, setClickBehavior] = React.useState(props.clickBehavior);
    ...
    return (
        <Card className="btn shadow p-3 mb-3 bg-white" style={{opacity: "0.6", borderRadius: "10px", border: "none"}} onClick={clickBehavior()}>
            <Row className="justify-content-start d-flex m-3">
                    <span className="mb-2" style={{fontSize: "0.9vw", fontWeight: "bold", color: "#1E4172"}}>
                        <FontAwesomeIcon icon={faStar}/> {title}
                    </span>
